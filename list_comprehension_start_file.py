...
[expression    iteration    conditior(optional)]

'''


'''
opigional_list=[x,y,z]
new_list = []
...
#get the square fo every even number from 0-10
...
newlist=[int(x) for x in numbers if int(x)%2==0 if x>0]
print(newlist)
...
numbers = [x for x in range(1,1001) if '4' in str(x)]
print(numbers)
## count how many times the word 'the' appears in the text file - 'sometext.txt'

infile=open('sometext.txt','r')
read=infile.readlines()
sentence=str(read).split()
the=[x for x in sentence if x =='the']
print(len(the))
...
extract=[int(x) for x in phrase if x.isdigit()]
print(extract)